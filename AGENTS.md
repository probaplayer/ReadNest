...
```
ReadNest/
├── Front-end/ReadNest-FE/ReadNest-FE/    # Blazor WASM frontend (.NET 10.0)
...
├── Back-end/ReadNest-BE/                 # .NET 10 backend API
...
- **Backend**: .NET 10, EF Core with SQLite, optional Redis for caching
...
- Frontend and backend `.csproj` files target .NET 10.0 — ensure the .NET 10 SDK is installed
